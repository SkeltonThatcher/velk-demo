...
if [ -d /vagrant/packages ] ; then
	...
	if [ ! -e /vagrant/packages/.remoterepotrue ] ; then
		...
		echo "Packages repo needs first time download"
		# if git exists on machine git clone repo in packages dir else pull in static tar ball via wget
		cd /vagrant/packages/
		...
		echo "Packages to support offline install already present"
	fi
	...
	# Configure logstash forwarder and use a local copy of init script
